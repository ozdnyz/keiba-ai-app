    ...
    try:
        if "gcp_service_account" in st.secrets:
            ...
            creds = Credentials.from_service_account_file(
                os.path.expanduser("~/key.json"), scopes=scopes
            )
            return gspread.authorize(creds)
        ...
        try:
            ...
            if all_vals and len(all_vals) > 1:
                ...
                df_target = pd.DataFrame(
                    target_rows, columns=headers[: len(target_rows[0])]
                )
            else:
                df_target = pd.DataFrame()
        except:
...
def show_flow_modal():
    ...
    <div style="background:#141A29; border:1px solid #2B354F; border-left:5px solid #6366F1; border-radius:10px; padding:14px 16px; margin-bottom:12px;">
        ...
        <div style="font-size:0.9rem; color:#F3F4F6; line-height:1.7;">
            <span style="color:#818CF8; font-weight:700;">・処理</span>: 全レース自動巡回 ➔ 芝・ダート黄金条件合致レースを抽出（👑 G1特化 / 平場ハイブリッド）<br>
    ...
    <div style="background:#141A29; border:1px solid #2B354F; border-left:5px solid #10B981; border-radius:10px; padding:14px 16px; margin-bottom:12px;">
        ...
        <div style="font-size:0.9rem; color:#F3F4F6; line-height:1.7;">
            <span style="color:#34D399; font-weight:700;">・処理</span>: 確定着順と配当を自動回収 ➔ AI買い目と照合<br>
...
if menu == "📊 ダッシュボード":
    ...
    if df_daily_log is not None and not df_daily_log.empty:
        ...
        for col in df_daily_calc.columns:
            if any(k in col for k in ["投資", "回収", "払戻", "レース", "的中", "R数"]):
                df_daily_calc[col] = pd.to_numeric(
                    df_daily_calc[col].astype(str).str.replace(",", ""), errors="coerce"
                ).fillna(0)
        ...
        if "ユーザー投資額" in df_daily_calc.columns:
            ...
            usr_tot_races = int(
                df_daily_calc.get(
                    "ユーザーレース数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            usr_tot_hits = int(
                df_daily_calc.get(
                    "ユーザー的中数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            ...
            usr_tot_races = int(
                df_daily_calc.get("レース数", pd.Series([0] * len(df_daily_calc))).sum()
            )
            usr_tot_hits = int(
                df_daily_calc.get("的中数", pd.Series([0] * len(df_daily_calc))).sum()
            )
        ...
        if (
            "AI投資額" in df_daily_calc.columns
            and df_daily_calc["AI投資額"].sum() > 0
        ):
            ai_all_inv = int(df_daily_calc["AI投資額"].sum())
            ai_all_ret = int(df_daily_calc["AI回収額"].sum())
            ai_all_races = int(
                df_daily_calc.get(
                    "AIレース数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            ai_all_hits = int(
                df_daily_calc.get(
                    "AI的中数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            has_real_ai = True
        elif "投資額" in df_daily_calc.columns:
        ...
        if "AI芝投資額" in df_daily_calc.columns:
            ...
            ai_turf_races = int(
                df_daily_calc.get(
                    "AI芝レース数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            ai_turf_hits = int(
                df_daily_calc.get(
                    "AI芝的中数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
        ...
        if "AIダート投資額" in df_daily_calc.columns:
            ...
            ai_dirt_races = int(
                df_daily_calc.get(
                    "AIダートレース数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )
            ai_dirt_hits = int(
                df_daily_calc.get(
                    "AIダート的中数", pd.Series([0] * len(df_daily_calc))
                ).sum()
            )

    ai_all_roi = (ai_all_ret / ai_all_inv * 100) if ai_all_inv > 0 else 0.0
    ai_all_hit_rate = (
        (ai_all_hits / ai_all_races * 100) if ai_all_races > 0 else 0.0
    )

    ai_turf_roi = (ai_turf_ret / ai_turf_inv * 100) if ai_turf_inv > 0 else 0.0
    ai_turf_hit_rate = (
        (ai_turf_hits / ai_turf_races * 100) if ai_turf_races > 0 else 0.0
    )

    ai_dirt_roi = (ai_dirt_ret / ai_dirt_inv * 100) if ai_dirt_inv > 0 else 0.0
    ai_dirt_hit_rate = (
        (ai_dirt_hits / ai_dirt_races * 100) if ai_dirt_races > 0 else 0.0
    )

    usr_roi = (usr_tot_ret / usr_tot_inv * 100) if usr_tot_inv > 0 else 0.0
    usr_hit_rate = (
        (usr_tot_hits / usr_tot_races * 100) if usr_tot_races > 0 else 0.0
    )
    ...
    if df_today is not None and not df_today.empty and "日付" in df_today.columns:
        ...
        venue_col = (
            "会場"
            if "会場" in df_today_latest.columns
            else ("競馬場" if "競馬場" in df_today_latest.columns else "")
        )
        if venue_col:
            today_race_count = len(
                df_today_latest[[venue_col, "レース名"]].drop_duplicates()
            )
        else:
    ...
    with c2:
        st.markdown(
            '<div class="right-card-header">👤 あなたの実戦 通算成績'
            " (実投票)</div>",
            unsafe_allow_html=True,
        )
        ...
        if has_real_usr and (usr_tot_inv > 0 or usr_tot_races > 0):
            ...
            usr_profit_class = (
                "kpi-profit-pos" if usr_profit >= 0 else "kpi-profit-neg"
            )
    ...
    if (
        (has_real_ai or has_real_usr)
        and df_daily_log is not None
        and not df_daily_log.empty
    ):
        df_plot = df_daily_calc.copy()
        df_plot["日付_dt"] = pd.to_datetime(df_plot["日付"], errors="coerce")
        df_plot = (
            df_plot.dropna(subset=["日付_dt"])
            .sort_values("日付_dt")
        ...
        if not df_plot.empty:
            ai_inv_col = (
                "AI投資額"
                if "AI投資額" in df_plot.columns
                else ("投資額" if "投資額" in df_plot.columns else "")
            )
            ai_ret_col = (
                "AI回収額"
                if "AI回収額" in df_plot.columns
                else ("払戻額" if "払戻額" in df_plot.columns else "")
            )
            usr_inv_col = (
                "ユーザー投資額"
                if "ユーザー投資額" in df_plot.columns
                else ("投資額" if "投資額" in df_plot.columns else "")
            )
            usr_ret_col = (
                "ユーザー回収額"
                if "ユーザー回収額" in df_plot.columns
                else ("払戻額" if "払戻額" in df_plot.columns else "")
            )
            ...
            if chart_mode == "月ごと (月別集計)":
                ...
                agg_cols = {
                    c: "sum"
                    for c in [ai_inv_col, ai_ret_col, usr_inv_col, usr_ret_col]
                    if c
                }
                sub_df = (
                    df_plot.groupby("集計キー", as_index=False)
                    .agg(agg_cols)
                    .sort_values("集計キー")
                )
                x_vals = sub_df["集計キー"].tolist()

                if ai_inv_col and ai_ret_col and sub_df[ai_inv_col].sum() > 0:
                    ai_vals = np.where(
                        sub_df[ai_inv_col] > 0,
                        (sub_df[ai_ret_col] / sub_df[ai_inv_col]) * 100,
                        0.0,
                    )
                    max_plot_val = max(
                        max_plot_val, float(np.max(ai_vals)) if len(ai_vals) > 0 else 0
                    )
                    fig.add_trace(
                        go.Scatter(
                            x=x_vals,
                            ...
                            hovertemplate="%{x}<br>AI月別回収率: %{y:.1f}%<extra></extra>",
                        )
                    )

                if usr_inv_col and usr_ret_col and sub_df[usr_inv_col].sum() > 0:
                    usr_vals = np.where(
                        sub_df[usr_inv_col] > 0,
                        (sub_df[usr_ret_col] / sub_df[usr_inv_col]) * 100,
                        0.0,
                    )
                    max_plot_val = max(
                        max_plot_val, float(np.max(usr_vals)) if len(usr_vals) > 0 else 0
                    )
                    fig.add_trace(
                        go.Scatter(
                            x=x_vals,
                ...
                if ai_inv_col and ai_ret_col and sub_df[ai_inv_col].sum() > 0:
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            line=dict(color="#6366F1", width=2.5),
                            marker=dict(size=7, color="#6366F1"),
                            hovertemplate="%{x}<br>AI年別回収率: %{y:.1f}%<extra></extra>",
                        )
                    )
            ...
            # 3. 日毎・累積推移モード（全体で通算累積を計算した上で切り出し）
                ...
                if ai_inv_col and ai_ret_col:
                    ...
                    df_plot["AI_CUM_R"] = df_plot[ai_ret_col].cumsum()
                    df_plot["AI_ROI"] = np.where(
                        df_plot["AI_CUM_I"] > 0,
                        (df_plot["AI_CUM_R"] / df_plot["AI_CUM_I"]) * 100,
                        0.0,
                    )

                if usr_inv_col and usr_ret_col:
                    df_plot["USR_CUM_I"] = df_plot[usr_inv_col].cumsum()
                    df_plot["USR_CUM_R"] = df_plot[usr_ret_col].cumsum()
                    df_plot["USR_ROI"] = np.where(
                        df_plot["USR_CUM_I"] > 0,
                        (df_plot["USR_CUM_R"] / df_plot["USR_CUM_I"]) * 100,
                        0.0,
                    )
                ...
                if (
                    ai_inv_col
                    and ai_ret_col
                    and "AI_ROI" in sub_df.columns
                    and sub_df[ai_inv_col].sum() > 0
                ):
                    ai_vals = sub_df["AI_ROI"].values
                    max_plot_val = max(
                        max_plot_val, float(np.max(ai_vals)) if len(ai_vals) > 0 else 0
                    )
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(size=7, color="#6366F1"),
                            hovertemplate="%{x}<br>AI通算: %{y:.1f}%<extra></extra>",
                        )
                    )
                ...
                if (
                    usr_inv_col
                    and usr_ret_col
                    and "USR_ROI" in sub_df.columns
                    and sub_df[usr_inv_col].sum() > 0
                ):
                    usr_vals = sub_df["USR_ROI"].values
                    max_plot_val = max(
                        max_plot_val, float(np.max(usr_vals)) if len(usr_vals) > 0 else 0
                    )
                    fig.add_trace(
                        go.Scatter(
                            ...
                            marker=dict(size=7, color="#10B981"),
                            hovertemplate="%{x}<br>実戦通算: %{y:.1f}%<extra></extra>",
                        )
                    )
...
# ==========================================
# 🎯 画面 2: 厳選勝負レース（ハイブリッド＆G1特化両立＋詳細アコーディオン付き）
# ==========================================
elif menu == "🎯 厳選勝負レース":
    st.markdown(
        '<div class="main-title">本日の厳選勝負レース</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="last-update">スプレッドシート「本日勝負レース」からリアルタイム取得中</div>',
        unsafe_allow_html=True,
    )

    if (
        df_target is not None
        and not df_target.empty
        and "日付" in df_target.columns
    ):
        latest_date_str = df_target["日付"].max()
        df_target_latest = df_target[df_target["日付"] == latest_date_str]
        ...
        for _, r in unique_races.iterrows():
            ...
            is_g1_race = "G1" in judge_str or "Ｇ１" in str(r["レース名"]) or "GI" in str(r["レース名"])
            badge_html = '<span class="race-badge-g1">👑 G1特化勝負</span>' if is_g1_race else '<span class="race-badge">👑 勝負レース</span>'
            ...
            # 券種別にワイド・馬単・その他（単勝・複勝・馬連・BOX）を柔軟に自動振り分け
            ...
            # 🛡️ ワイド系（平場ワイド5点 / G1ワイドBOX・穴流し）
            if not df_wide.empty:
                w_title = "🛡️ 【G1特化】ワイド / ワイドBOX" if is_g1_race else "🛡️ 【守り】ワイド5点（各100円・計500円 / 的中率55%・最大10連敗防衛）"
                st.markdown(
                    f'<div style="font-size:0.85rem; font-weight:700; color:#10B981; margin-bottom:6px;">{w_title}</div>',
                    unsafe_allow_html=True,
                )
            ...
            if not df_umatan.empty:
                u_title = "🚀 【G1特化】馬単1着固定3点（回収爆発型）" if is_g1_race else "🚀 【攻め】馬単1着固定3点（各100円・計300円 / 回収率133%ゾーン直撃）"
                st.markdown(
                    f'<div style="font-size:0.85rem; font-weight:700; color:#F59E0B; margin-top:6px; margin-bottom:6px;">{u_title}</div>',
                    unsafe_allow_html=True,
                )
            ...
            # 🎯 単勝・複勝・馬連系（G1王道馬連・単複防衛など）
            if not df_other.empty:
                o_title = "🎯 【G1特化】王道馬連 / 単勝 / 複勝" if is_g1_race else "🎯 推奨買い目一覧"
                st.markdown(
                    f'<div style="font-size:0.85rem; font-weight:700; color:#3B82F6; margin-bottom:6px;">{o_title}</div>',
                    unsafe_allow_html=True,
                )
...
# ==========================================
    ...
    st.markdown(
        '<div class="main-title">全レース出馬表 ＆ AI評価印</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="last-update">馬場状態を切り替えて、レースの買い/見送り判定と各頭の印をシミュレーションできます</div>',
        unsafe_allow_html=True,
    )
    ...
    if df_today is not None and not df_today.empty and "日付" in df_today.columns:
        ...
        if "馬場" in df_today_latest.columns and (
            df_today_latest["馬場"] == b_val
        ).any():
        ...
        for i, venue in enumerate(venues):
            with tabs[i]:
                ...
                for rname in race_names:
                    ...
                    cond = (
                        str(sub_df.iloc[0]["芝・ダ・障"])
                        + str(sub_df.iloc[0]["距離"])
                        + "m"
                    )
                    ...
                    with st.expander(f"🏁 {venue} {rname} （{cond}）"):
                        if "レース判定" in sub_df.columns:
                            ...
                            if "買い" in r_judge:
                                st.markdown(
                                    f'<div class="status-badge-buy">🎯 {r_judge} ｜'
                                    f" {r_detail}</div>",
                                    unsafe_allow_html=True,
                                )
                            else:
                                st.markdown(
                                    f'<div class="status-badge-skip">✋ {r_judge} ｜ 理由:'
                                    f" {r_detail}</div>",
                                    unsafe_allow_html=True,
                                )
                        ...
                        display_df["馬番"] = pd.to_numeric(
                            display_df["馬番"], errors="coerce"
                        )
                            ...
                            for col in actual_cols:
                                ...
                                if col == "印":
                                    if val == "◎":
                                        ...
                                        val = (
                                            "<span style='color: #3B82F6; font-weight: 900;"
                                            " font-size: 1.1rem;'>◯</span>"
                                        )
                                    elif val == "▲":
                                        val = (
                                            "<span style='color: #10B981; font-weight: 900;"
                                            " font-size: 1.1rem;'>▲</span>"
                                        )
                                    elif "△" in str(val):
                                        val = (
                                            f"<span style='color: #F59E0B; font-weight: 900;"
                                            f" font-size: 1.05rem;'>{val}</span>"
                                        )
...
# ==========================================
    ...
    st.markdown(
        '<div class="main-title">日次実収支の記録</div>', unsafe_allow_html=True
    )
    st.markdown(
        '<div class="last-update">一日の終わりに、今日の購入実績（金額・レース数・的中数）を入力して保存してください</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.form("shushi_form"):
        st.markdown(
            '<div style="font-size:0.95rem; font-weight:700; color:#FFFFFF;'
            ' margin-bottom:6px;">🗓️ 競馬開催日</div>',
            unsafe_allow_html=True,
        )
        target_date = st.date_input(
            "開催日", datetime.now(), label_visibility="collapsed"
        )

        st.markdown(
            '<div style="font-size:0.9rem; font-weight:700; color:#818CF8;'
            ' margin-top:14px; margin-bottom:8px;">【収支金額】</div>',
            unsafe_allow_html=True,
        )
        ...
        with c_in1:
            st.markdown(
                ...
                ' margin-bottom:4px;">💸 今日の総購入額 (円)</div>',
                unsafe_allow_html=True,
            )
            usr_inv = st.number_input(
                "総購入額", min_value=0, value=0, step=100, label_visibility="collapsed"
            )
        with c_in2:
            st.markdown(
                '<div style="font-size:0.9rem; font-weight:700; color:#FFFFFF;'
                ' margin-bottom:4px;">💰 今日の総払戻額 (円)</div>',
                unsafe_allow_html=True,
            )
            usr_ret = st.number_input(
                "総払戻額", min_value=0, value=0, step=100, label_visibility="collapsed"
            )

        st.markdown(
            '<div style="font-size:0.9rem; font-weight:700; color:#34D399;'
            ' margin-top:14px; margin-bottom:8px;">【的中率カウント】</div>',
            unsafe_allow_html=True,
        )
        ...
        with c_in3:
            st.markdown(
                '<div style="font-size:0.9rem; font-weight:700; color:#FFFFFF;'
                ' margin-bottom:4px;">🏇 実際に購入したレース数</div>',
                unsafe_allow_html=True,
            )
            usr_races = st.number_input(
                "購入レース数",
                min_value=0,
                value=0,
                step=1,
        ...
        with c_in4:
            st.markdown(
                '<div style="font-size:0.9rem; font-weight:700; color:#FFFFFF;'
                ' margin-bottom:4px;">🎯 的中したレース数</div>',
                unsafe_allow_html=True,
            )
            usr_hits = st.number_input(
                "的中レース数",
                min_value=0,
                value=0,
                step=1,
            ...
            if gc:
                ...
                try:
                    ...
                    def get_col(name, default_idx):
                        return (
                            headers.index(name) + 1 if name in headers else default_idx
                        )
                    ...
                    if found_row != -1:
                        ...
                        st.success(
                            f"✅ {date_str} の実戦記録を更新しました！（投資: {usr_inv:,}円 /"
                            f" 回収: {usr_ret:,}円 / 的中: {usr_hits}/{usr_races}R）"
                        )
                    else:
                        new_row = [date_str] + [0] * max(0, len(headers) - 1)
                        if len(new_row) >= col_u_inv:
                            new_row[col_u_inv - 1] = usr_inv
                        if len(new_row) >= col_u_ret:
                            new_row[col_u_ret - 1] = usr_ret
                        if len(new_row) >= col_u_races:
                            new_row[col_u_races - 1] = usr_races
                        if len(new_row) >= col_u_hits:
                            new_row[col_u_hits - 1] = usr_hits
                        ws.append_row(new_row)
                        st.success(
                            f"✅ {date_str} の実戦記録を新規保存しました！（投資: {usr_inv:,}円 /"
                            f" 回収: {usr_ret:,}円 / 的中: {usr_hits}/{usr_races}R）"
                        )
...
# ==========================================
    ...
    st.markdown(
        '<div class="main-title">ターミナル操作マニュアル</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="last-update">各枠右上のコピーボタンを押してターミナルに貼り付けてください</div>',
        unsafe_allow_html=True,
    )
    ...
    <div style="background:#141A29; border:1px solid #1E273D; border-radius:12px; padding:1.1rem; margin-bottom:1.2rem;">
        <h4 style="color:#FFFFFF; margin-top:0; font-size:1rem;">⚡ 1. 朝の予想手動実行（全レース巡回 ＆ 買い目自動選定）</h4>
    """,
        unsafe_allow_html=True,
    )
    st.code(
        "cd /home/ozdnyzww1 && /home/ozdnyzww1/keiba_env/bin/python3 run.py",
        language="bash",
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.caption("※芝・ダート・G1を自動判別し、最適化された買い目を出力")
    st.markdown("</div>", unsafe_allow_html=True)

    st.markdown(
        """
    <div style="background:#141A29; border:1px solid #1E273D; border-radius:12px; padding:1.1rem; margin-bottom:1.2rem;">
        <h4 style="color:#FFFFFF; margin-top:0; font-size:1rem;">🏁 3. 【手動結果回収】的中照合 ＆ 日次収支自動集計</h4>
    """,
        unsafe_allow_html=True,
    )
    st.code(
        "cd /home/ozdnyzww1 && /home/ozdnyzww1/keiba_env/bin/python3 result.py",
        language="bash",
    )
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.code(
        "killall -9 chromium chromium-driver chromedriver chrome 2>/dev/null",
        language="bash",
    )
    st.markdown("</div>", unsafe_allow_html=True)

# ==========================================
# 🗄️ 過去データ分析（最新ハイブリッド実績 ＆ 全G1構造戦略カルテ）
# ==========================================
elif menu == "🗄️ 過去データ分析":
    st.markdown(
        '<div class="main-title">【新】過去データバックテスト分析（2022〜2026年）</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="last-update">5年間・21万行の実データ検証による【最終確定】勝負条件</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        ' margin-top:1.5rem; margin-bottom:6px;">🏛️ 全24 G1レース別 構造・傾向戦略カルテ</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="last-update">前走格付け・コース枠順バイアス・波乱度別 最適券種一覧</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        ...
        <table style="width:100%; border-collapse: collapse; color: #F3F4F6; background-color: #141A29; font-size: 0.92rem;">
            <thead>
                <tr style="background-color: #0E1322; color: #94A3B8; border-bottom: 2px solid #1E273D; text-align: left;">
                    <th style="padding: 10px 12px;">分類</th>
                    <th style="padding: 10px 12px;">レース名</th>
                    <th style="padding: 10px 12px;">👑 採用券種</th>
                    <th style="padding: 10px 12px;">投資点数</th>
                    <th style="padding: 10px 12px;">構造的理由・バイアス</th>
                </tr>
            </thead>
            <tbody>
                <tr style="border-bottom: 1px solid #1E273D;"><td rowspan="5" style="padding:10px; font-weight:700; color:#10B981;">中長距離・中穴直撃型</td><td style="padding:8px 10px;">有馬記念</td><td style="padding:8px 10px; font-weight:700;">上位4頭ワイドBOX</td><td style="padding:8px 10px;">6点 (600円)</td><td style="padding:8px 10px;">中山2500m内枠バイアス＋先行伏兵の台頭</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">天皇賞(秋)</td><td style="padding:8px 10px; font-weight:700;">上位4頭ワイドBOX</td><td style="padding:8px 10px;">6点 (600円)</td><td style="padding:8px 10px;">東京2000m外枠不利。中穴馬のダブル的中狙い</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">菊花賞・天皇賞(春)</td><td style="padding:8px 10px; font-weight:700;">上位4頭ワイドBOX</td><td style="padding:8px 10px;">6点 (600円)</td><td style="padding:8px 10px;">スタミナ持久戦。前走前哨戦好走馬を広く拾う</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">スプリンターズS・高松宮</td><td style="padding:8px 10px; font-weight:700;">上位4頭ワイドBOX</td><td style="padding:8px 10px;">6点 (600円)</td><td style="padding:8px 10px;">電撃戦・道悪の紛れをBOXで防衛・高配当狙い</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">秋華賞・宝塚・大阪杯</td><td style="padding:8px 10px; font-weight:700;">上位4頭ワイドBOX</td><td style="padding:8px 10px;">6点 (600円)</td><td style="padding:8px 10px;">内回り立ち回り巧者の激走を総取り</td></tr>
                
                <tr style="border-bottom: 1px solid #1E273D;"><td rowspan="4" style="padding:10px; font-weight:700; color:#3B82F6;">王道・実力決着型</td><td style="padding:8px 10px;">日本ダービー</td><td style="padding:8px 10px; font-weight:700;">馬連3点</td><td style="padding:8px 10px;">3点 (300円)</td><td style="padding:8px 10px;">軸複勝率100%。ごまかし利かない能力上位ワンツー</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">オークス・ジャパンC</td><td style="padding:8px 10px; font-weight:700;">馬連3〜5点</td><td style="padding:8px 10px;">3〜5点</td><td style="padding:8px 10px;">府中芝2400mの王道決着。余計な買い目を削ぎ落とす</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">フェブラリーS</td><td style="padding:8px 10px; font-weight:700;">馬単3点</td><td style="padding:8px 10px;">3点 (300円)</td><td style="padding:8px 10px;">ダート血統上位馬の一着固定押し切り</td></tr>
                <tr style="border-bottom: 1px solid #1E273D;"><td style="padding:8px 10px;">朝日杯FS・チャンピオンズ</td><td style="padding:8px 10px; font-weight:700;">馬単3点 / 馬連5点</td><td style="padding:8px 10px;">3〜5点</td><td style="padding:8px 10px;">完成度上位・ダート実績馬の本線決着</td></tr>

                <tr><td rowspan="2" style="padding:10px; font-weight:700; color:#818CF8;">ピンポイント単複防衛</td><td style="padding:8px 10px;">皐月賞・ホープフルS</td><td style="padding:8px 10px; font-weight:700;">単勝◎</td><td style="padding:8px 10px;">1点 (100円)</td><td style="padding:8px 10px;">相手が絞りにくい中山2000m。実力軸馬の単勝勝負</td></tr>
                <tr><td style="padding:8px 10px;">安田記念・マイルCS</td><td style="padding:8px 10px; font-weight:700;">複勝◎</td><td style="padding:8px 10px;">1点 (100円)</td><td style="padding:8px 10px;">極限の混戦マイル。手を出さず複勝1点で手堅く防衛</td></tr>
            </tbody>
        </table>
        </div>
        """,
...
# ==========================================
    ...
    st.markdown(
        '<div class="main-title">Google スプレッドシート連携ステータス</div>',
        unsafe_allow_html=True,
    )
    gc = get_gspread_client()
    if gc:
        st.success(
            f"🔐 連携成功: 「{SS_NAME}」と正常にリアルタイム同期しています。"
        )
